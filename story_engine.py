...
# Explicit opt-in phrases for proactive storytelling.
...
def choose_story(preferred_topic=None):
    ...
    if preferred_topic:
        ...
        matching = [
            ...
            if topic in _normalize(story["topic"] + " " + story["tags"])
        ]
        if matching:
            candidates = matching
...
def build_story_prompt(story, proactive=False):
    if not story:
        return ""

    mode_text = (
        "Tiểu Vũ chủ động đề nghị kể chuyện vì cuộc trò chuyện đang có khoảng lặng."
        if proactive else
        "Lão sư vừa yêu cầu kể chuyện."
    )
...
- Mỗi lượt nói một đoạn vừa phải, khoảng 30–90 giây. Sau đó dừng để Lão sư có thể phản ứng hoặc nói "kể tiếp".
- Nếu Lão sư nói "kể tiếp", tiếp tục câu chuyện đang kể thay vì chọn chuyện mới.
"""


def get_story_prompt(preferred_topic=None, proactive=False):
    global _last_story_at
    story = choose_story(preferred_topic)
    if not story:
        return None
    _last_story_at = time.monotonic()
    return build_story_prompt(story, proactive=proactive)
